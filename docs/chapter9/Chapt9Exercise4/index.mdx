...
The stiff nature of laser rate equations requires:
\begin{equation}
\Delta t < \min\left(\tau_p, \frac{1}{\omega_R}\right)
\end{equation}
With $\tau_p \approx 1.7$ ps and $\omega_R \sim 10$ GHz, the 1 ps time step is adequate.

%==============================================================================
\section{Physical Interpretation}
%==============================================================================

\subsection{Turn-On Dynamics}

The simulation reveals several distinct phases:

\textbf{1. Pre-injection ($t < 2.5$ ns):} Both $n$ and $S$ remain at zero.

\textbf{2. Carrier buildup ($2.5 < t < t_{th}$ ns):} After current injection, carriers accumulate:
\begin{equation}
n(t) \approx \frac{I\tau_n}{eV}\left(1 - e^{-t/\tau_n}\right)
\end{equation}

\textbf{3. Threshold crossing:} When $n = n_{th}$, gain equals loss and photon density begins exponential growth.

\textbf{4. Relaxation oscillations:} The carrier-photon coupling produces damped oscillations at frequency:
\begin{equation}
f_R = \frac{1}{2\pi}\sqrt{\frac{\Gamma g_0 v_g S_0}{n_g}} \approx 2-5 \text{ GHz}
\end{equation}

\textbf{5. Steady state:} Carrier density clamps near $n_{th}$, photon density reaches equilibrium.

\subsection{Gain Compression Effects}

The factor $(1 - \varepsilon S)$ models spectral hole burning:
\begin{equation}
g_{eff} = \Gamma g_0 (n - n_0) \cdot \frac{1}{1 + \varepsilon S}
\end{equation}
This limits maximum gain and affects modulation bandwidth.

%==============================================================================
\section{Summary}
%==============================================================================

This code implements a complete single-mode laser rate equation solver featuring:
\begin{itemize}
\item Fourth-order Runge-Kutta integration for high accuracy
\item Realistic device parameters (InGaAsP 1310 nm laser)
\item ABC recombination model
\item Gain compression for nonlinear saturation
\item Spontaneous emission coupling to lasing mode
\item Power output calculation from cavity photon density
\end{itemize}

The simulation captures key laser dynamics: turn-on delay, relaxation oscillations at GHz frequencies, and steady-state operation above threshold.
